...
var IndexRoute = (function () {
    ...
    Object.defineProperty(IndexRoute.prototype, "clientes", {
        get: function () {
            ...
            return function (req, res) {
                if (req.session.user) {
                    _this.resolveRequestBank("select * from usuario where id_adm = $1", req.session.user.id).then(function (result) {
                        result['rows'].forEach(function (client) {
                            client.datanascimento = _this.inverterData(client.datanascimento.toISOString().split('T')[0]);
                            client.ultimoacesso = _this.inverterData(client.ultimoacesso.toISOString().split('T')[0]);
                        });
                        res.render('clientes', { clientes_table: result['rows'], clientes: req.session.clientes != undefined ? req.session.clientes : '' });
                    });
                }
                else
                    res.render("403");
        ...
        get: function () {
            ...
            return function (req, res) { return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    if (req.session.user) {
                        promise1 = this.resolveRequestBank("select * from usuario where id = $1", req.session.user.id);
                        ...
                        promise3 = this.resolveRequestBank("select * from usuario where id_adm = $1", req.session.user.id);
                        Promise.all([promise1, promise2, promise3]).then(function (data) {
                            res.render('painel', { usuario: req.session.user, mensagens: data[1]['rows'][0].mensagens, clientes: req.session.clientes != undefined ? req.session.clientes : '', tarefas: false, desafios: false });
